...
b){return a+b},0)}var k=b.isArray;a=a.seriesType;a("cci","sma",{params:{period:14}},{getValues:function(a,b){b=b.period;var l=a.xData,t=(a=a.yData)?a.length:0,m=[],e,d=1,n=[],p=[],q=[],c,f;if(l.length<=b||!k(a[0])||4!==a[0].length)return!1;for(;d<b;)c=a[d-1],m.push((c[1]+c[2]+c[3])/3),d++;for(d=b;d<=t;d++){c=a[d-1];c=(c[1]+c[2]+c[3])/3;f=m.push(c);e=m.slice(f-b);f=h(e)/b;var u=e.length,r=0,g;for(g=0;g<u;g++)r+=Math.abs(f-e[g]);e=r/b;c=(c-f)/(.015*e);n.push([l[d-1],c]);p.push(l[d-1]);q.push(c)}return{values:n,

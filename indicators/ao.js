...
a=a.lowerBarColor,d=g[0];if(!this.userOptions.color&&d)for(d.color=b,d=1;d<g.length;d++)g[d].color=g[d].y>g[d-1].y?b:g[d].y<g[d-1].y?a:g[d-1].color},getValues:function(a){var b=a.xData||[];a=a.yData||[];var q=a.length,d=[],n=[],p=[],h=0,k=0,e,l,c;if(34>=b.length||!m(a[0])||4!==a[0].length)return!1;for(c=0;33>c;c++)e=(a[c][1]+a[c][2])/2,29<=c&&(h=f(h+e)),k=f(k+e);for(c=33;c<q;c++)e=(a[c][1]+a[c][2])/2,h=f(h+e),k=f(k+e),e=h/5,l=k/34,e=f(e-l),d.push([b[c],e]),n.push(b[c]),p.push(e),e=c+1-5,l=c+1-34,
h=f(h-(a[e][1]+a[e][2])/2),k=f(k-(a[l][1]+a[l][2])/2);return{values:d,xData:n,yData:p}}})});b(a,"masters/indicators/ao.src.js",[],function(){})});

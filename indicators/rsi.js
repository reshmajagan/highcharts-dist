...
a.seriesType("rsi","sma",{params:{period:14,decimals:4}},{getValues:function(a,b){var d=b.period,n=a.xData,m=(a=a.yData)?a.length:0;b=b.decimals;var c=1,p=[],q=[],r=[],e=0,g=0,f,h,k;if(n.length<d||!l(a[0])||4!==a[0].length)return!1;for(;c<d;)f=parseFloat((a[c][3]-a[c-1][3]).toFixed(b)),0<f?e+=f:g+=Math.abs(f),c++;h=parseFloat((e/(d-1)).toFixed(b));for(k=parseFloat((g/(d-1)).toFixed(b));c<m;c++)f=parseFloat((a[c][3]-a[c-1][3]).toFixed(b)),0<f?(e=f,g=0):(e=0,g=Math.abs(f)),h=parseFloat(((h*(d-1)+e)/
d).toFixed(b)),k=parseFloat(((k*(d-1)+g)/d).toFixed(b)),e=0===k?100:0===h?0:parseFloat((100-100/(1+h/k)).toFixed(b)),p.push([n[c],e]),q.push(n[c]),r.push(e);return{values:p,xData:q,yData:r}}})});b(a,"masters/indicators/rsi.src.js",[],function(){})});
